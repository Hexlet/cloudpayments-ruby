# frozen_string_literal: true

module Cloudpayments
  module Models
    # @see Cloudpayments::Resources::Subscriptions#update
    class SubscriptionUpdateResponse < Cloudpayments::Internal::Type::BaseModel
      # @!attribute success
      #   Результат выполнения запроса
      #
      #   @return [Boolean]
      required :success, Cloudpayments::Internal::Type::Boolean, api_name: :Success

      # @!attribute message
      #   Описание ошибки (при наличии)
      #
      #   @return [String, nil]
      optional :message, String, api_name: :Message

      # @!attribute model
      #   Модель подписки на рекуррентные платежи
      #
      #   @return [Cloudpayments::Models::Subscription, nil]
      optional :model, -> { Cloudpayments::Subscription }, api_name: :Model

      # @!method initialize(success:, message: nil, model: nil)
      #   Базовый ответ API
      #
      #   @param success [Boolean] Результат выполнения запроса
      #
      #   @param message [String] Описание ошибки (при наличии)
      #
      #   @param model [Cloudpayments::Models::Subscription] Модель подписки на рекуррентные платежи
    end
  end
end
